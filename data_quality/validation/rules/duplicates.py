...
class DuplicateTransactionRule(BaseValidationRule):
    ...
    def validate(self, connection) -> ValidationResult:
        """
        Find duplicate transactions for the given connection.
        ...
        try:
            ...
            logger.info(f"Running duplicate transaction validation for connection {connection_id}")
            
            if not connection:
                raise ValueError("Integration cannot be None")
            
            # Get base queryset of transactions for this connection
            queryset = Transaction.objects.filter(connection=connection)
            ...
            if not duplicates_data:
                logger.info(f"No duplicate transactions found for connection {connection.id}")
            ...
            duplicate_groups = self._process_duplicate_groups(duplicates_data, connection)
    ...
    def _process_duplicate_groups(self, duplicates_data: List[Dict], connection) -> List[Dict]:
        ...
        for group_data in duplicates_data:
            ...
            group_filter = Q(connection=connection)
            ...
            transactions = Transaction.objects.filter(group_filter).order_by('-date', '-created_at')

...
class ValidationRuleRegistry:
    ...
    @classmethod
    def get_enabled_rules_for_integration(cls, integration) -> List[BaseValidationRule]:
        """
        Get all enabled validation rules for a specific integration.
        ...
        for rule_name, rule_class in cls._rules.items():
            ...
            if rule_instance.is_enabled_for_integration(integration):
                # Get integration-specific config
                config = rule_instance.get_config_for_integration(integration)
    ...
    def _discover_and_load_rules(cls) -> None:
        ...
        try:
            ...
            import integrations.validation.rules
            
            # Walk through all modules in the rules package
            rules_package = integrations.validation.rules
            ...
            try:
                from integrations.validation.rules import duplicates
